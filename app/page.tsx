...
type RecordItem = {
  ...
  note: string; evidenceName?: string; evidenceHash?: string;
...
const DAILY_EXERCISE_TARGET = 100;
...
function mentionsToeic(value: unknown) { return /toeic|토익|part [23457]|모의고사/i.test(String(value || "")); }
function supportedRecords(records: RecordItem[]) { return records.filter(record => !mentionsToeic(record.kind)); }
...
export default function Home() {
    ...
    (async () => {
      ...
      if (!first.exists()) {
        ...
        const migrated: AppState = legacy ? {
          records: supportedRecords(legacy.records || []),
          audits: (legacy.audits || []).filter((audit: Audit) => !mentionsToeic(audit.summary)),
          posts: (legacy.posts || []).filter((post: BlogPost) => !mentionsToeic(`${post.title} ${post.body}`)),
      ...
      unsubscribe = onSnapshot(userRef, { includeMetadataChanges: true }, snapshot => {
        ...
        const originalAudits = data.audits || [];
        const originalPosts = data.posts || [];
        const remote: AppState = {
          records: supportedRecords(originalRecords),
          audits: originalAudits.filter((audit: Audit) => !mentionsToeic(audit.summary)),
          posts: originalPosts.filter((post: BlogPost) => !mentionsToeic(`${post.title} ${post.body}`)),
          emergency: data.emergency || [], startedAt: data.startedAt,
        };
        applyingRemote.current = true;
        lastSerialized.current = JSON.stringify({
          records: originalRecords, audits: originalAudits, posts: originalPosts,
          emergency: data.emergency || [], startedAt: data.startedAt,
        });
        setState(remote);
        setReady(true);
  ...
  useEffect(() => {
    ...
    const pushupTotal = accepted.filter(r => r.kind === "pushup").reduce((n,r)=>n+(r.amount||0),0);
    const situpTotal = accepted.filter(r => r.kind === "situp").reduce((n,r)=>n+(r.amount||0),0);
    ...
    const failures = [pushupTotal < DAILY_EXERCISE_TARGET ? `푸시업 ${pushupTotal}/${DAILY_EXERCISE_TARGET}개` : "", situpTotal < DAILY_EXERCISE_TARGET ? `윗몸일으키기 ${situpTotal}/${DAILY_EXERCISE_TARGET}개` : "", stockProofs < 2 ? `주식 증빙 ${stockProofs}/2건` : "", emergencyFailures?`비상 루틴 ${emergencyFailures}회 미완료`:""].filter(Boolean);
  ...
  const situps = acceptedToday.filter(r => r.kind === "situp").reduce((n, r) => n + (r.amount || 0), 0);
  ...
  return <main className="shell">
    ...
    <nav>{[["today","오늘"],["emergency","비상 루틴"],["record","기록"],["review","검토"],["blog","공개 회고"],["audit","감사 로그"]].map(([id,t]) => <button key={id} className={tab===id?"active":""} onClick={() => setTab(id)}>{t}</button>)}</nav>
    {message && <div className="notice" role="status">{message}<button onClick={() => setMessage("")}>닫기</button></div>}
    {tab === "today" && <Today pushups={pushups} situps={situps} stockEvidence={stockEvidence} records={state.records} />}
...
function Today({ pushups, situps, stockEvidence, records }: { pushups:number; situps:number; stockEvidence:number; records:RecordItem[] }) {
  const pending = records.filter(r => r.status === "submitted" || r.status === "needs_review").length;
  return <section><div className="hero-grid"><article className="metric primary"><span>푸시업</span><strong>{pushups}<small>/{DAILY_EXERCISE_TARGET}</small></strong><progress value={pushups} max={DAILY_EXERCISE_TARGET}/><p>{Math.max(0,DAILY_EXERCISE_TARGET-pushups)}개 남음</p></article><article className="metric"><span>윗몸일으키기</span><strong>{situps}<small>/{DAILY_EXERCISE_TARGET}</small></strong><progress value={situps} max={DAILY_EXERCISE_TARGET}/><p>{Math.max(0,DAILY_EXERCISE_TARGET-situps)}개 남음</p></article><article className="metric"><span>주식 증빙</span><strong>{stockEvidence}<small>/2장</small></strong><progress value={stockEvidence} max={2}/><p>주문·체결 + 보유 내역</p></article><article className="metric"><span>검토 대기</span><strong>{pending}<small>건</small></strong><p>불확실한 기록은 성공으로 세지 않습니다.</p></article></div><div className="panel"><div className="section-title"><div><p className="eyebrow">우선순위</p><h2>지금 빠져나갈 구멍</h2></div></div><ol className="risks">{pushups < DAILY_EXERCISE_TARGET && <li><b>푸시업 {DAILY_EXERCISE_TARGET-pushups}개 부족</b><span>세트 기록과 증빙을 함께 제출하세요.</span></li>}{situps < DAILY_EXERCISE_TARGET && <li><b>윗몸일으키기 {DAILY_EXERCISE_TARGET-situps}개 부족</b><span>세트 기록과 증빙을 함께 제출하세요.</span></li>}{stockEvidence < 2 && <li><b>주식 화면 {2-stockEvidence}장 부족</b><span>무매매도 주문·체결 내역으로 증명해야 합니다.</span></li>}</ol></div></section>;
}
...
function Emergency({sessions,setState}:{sessions:EmergencySession[];setState:React.Dispatch<React.SetStateAction<AppState>>}){
  ...
  return <section className="panel emergency"><div className="section-title"><div><p className="eyebrow">CONDITION RESET</p><h2>생각을 늘리지 말고 정한 행동을 실행</h2></div></div>
    {!active&&<form className="record-form" onSubmit={start}><label>루틴 직후 시작할 업무<input value={task} onChange={e=>setTask(e.target.value)} required minLength={3} placeholder="예: 푸시업 20개 시작"/><small>시작 후 수정·삭제할 수 없습니다.</small></label><ConditionFields value={before} onChange={setBefore}/><button className="submit">비상 루틴 시작</button></form>}
...
function label(k:Kind){return k==="pushup"?"푸시업":k==="situp"?"윗몸일으키기":"주식 규칙"}
